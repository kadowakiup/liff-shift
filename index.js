...
window.onload = async function () {
  ...
  saveEdit.addEventListener("click", async () => {
    ...
    const now = new Date();
    const startDt = new Date(`${selectedDate}T${newStart}:00`);
    const endDt = new Date(`${selectedDate}T${newEnd}:00`);
    const originalEndDt = new Date(`${selectedDate}T${originalEnd}:00`);

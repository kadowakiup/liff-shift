...
window.onload = async function () {
  ...
  function calcShiftMinutes(dateKey, shift) {
    ...
    const start = new Date(`${dateKey}T${shift.start}:00`);
    const end = new Date(`${dateKey}T${shift.end}:00`);
  ...
  function calcMonthlyWorkTime() {
    ...
    for (const dateKey in shiftData) {
      ...
      shiftData[dateKey].forEach(s => {
        total += calcShiftMinutes(dateKey, s);
      });
    }
  ...
  function updateWorktimeDisplay() {
    workTimeSpan.textContent = calcMonthlyWorkTime();
  }
  ...
  async function reloadShifts() {
    ...
    const data = await fetchJson(
      GAS_URL +
      "?action=fetch&userId=" + profile.userId +
      "&name=" + encodeURIComponent(profile.displayName)
    );
  ...
  saveEdit.addEventListener("click", async () => {
    ...
    if (!selectedShiftId) {
      ...
      await fetchJson(
        GAS_URL +
        `?action=addShift&userId=${profile.userId}&date=${selectedDateStr}&start=${start}&end=${end}`
      );

      if (!shiftData[selectedDateStr]) shiftData[selectedDateStr] = [];

      shiftData[selectedDateStr].push({
        id: "temp_" + Date.now(),
        start,
        end,
        state: ""
      ...
      await fetchJson(
        GAS_URL +
        `?action=update&userId=${profile.userId}&shiftId=${selectedShiftId}&date=${selectedDateStr}&start=${start}&end=${end}`
      );

      const found = findShiftById(selectedShiftId);
      if (found) {
        found.shift.start = start;
        found.shift.end = end;
      }
    }
  ...
  btnDelete.addEventListener("click", async () => {
    ...
    await fetchJson(
      GAS_URL +
      `?action=deleteOrAbsent&userId=${profile.userId}&shiftId=${selectedShiftId}&date=${selectedDateStr}`
    );
  ...
  function findShiftById(id) {
    for (const d in shiftData) {
      const f = shiftData[d].find(s => s.id === id);
      if (f) return { shift: f };
    }
    return null;
  }
  ...
  function generateCalendar() {
    ...
    for (let i = 1; i <= 31; i++) {
      ...
      shifts.forEach(s => {
        ...
        div.textContent = s.start && s.end ? `${s.start}-${s.end}` : s.state;
  ...
  updateButton.onclick = reloadShifts;
};
